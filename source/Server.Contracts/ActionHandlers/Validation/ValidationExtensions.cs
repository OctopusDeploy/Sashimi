    ...
    public static class ValidationExtensions
    {
        ...
        public static IRuleBuilderOptions<T, IEnumerable<PackageReference>> MustHaveExactlyOnePackage<T>(this IRuleBuilderInitial<T, IEnumerable<PackageReference>> ruleBuilder, string errorMessage)
        {
            return ruleBuilder
                ...
                .Must(packages =>
                {
                    var packageReferences = packages as PackageReference[] ?? packages.ToArray();

                    var firstPackage = packageReferences.FirstOrDefault();

                    return firstPackage != null && packageReferences.Length == 1 &&
